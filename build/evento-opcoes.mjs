// ---------------------------------------------------------------------------
// Pé de Manga — gerador de MENU DE EVENTO por OPÇÕES (ex.: Happy Hour)
//
// Evento de pacote fechado: SEM preços. Cada OPÇÃO é um pacote completo, com
// suas SEÇÕES (ex.: Bebidas, Cozinha) e itens (texto). A4 HORIZONTAL dividido
// em 3 partes iguais (3 cópias). Reaproveita o design monocromático aprovado
// do menu de evento (build/evento.mjs).
//
//   node build/evento-opcoes.mjs [slug]      (default: happy-hour)
//
// Saídas (output/): <slug>-<idOpcao>.pdf (uma por opção) · <slug>-geral.pdf
// ---------------------------------------------------------------------------
...
// Quando o logo é um wordmark (já traz "Bar & Restaurante"), usa-o maior e sem
// o texto redundante no cabeçalho.
const WORDMARK = !!data.wordmark;
const esc = (s) => String(s == null ? '' : s).replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');

// ---- validação ------------------------------------------------------------
function die(msg) { console.error('✗ ' + msg); process.exit(1); }
if (!Array.isArray(data.opcoes) || !data.opcoes.length) die('nenhuma opção definida (opcoes).');
data.opcoes.forEach((op, i) => {
  if (!op.titulo) die(`opção ${i + 1} sem título.`);
  if (!Array.isArray(op.secoes) || !op.secoes.length) die(`opção "${op.titulo}" sem seções.`);
});
...
function panel(op) {
  return `<div class="panel">
    <div class="panel-head${WORDMARK ? ' wm' : ''}"><div class="logo"></div>${WORDMARK ? '' : '<div>Bar &amp; Restaurante</div>'}</div>
    <div class="panel-body">
      <div class="kicker">${esc(data.subtitulo)}</div>
      <h1>${esc(data.titulo)}</h1>
      <div class="option-row"><strong>${esc(op.titulo)}</strong><span>${esc(data.periodo)}</span></div>
      <div class="lead">${esc(op.chamada)}</div>
      <div class="sections">
        ${(op.secoes || []).map((s) => sec(s.titulo, '', s.itens)).join('')}
      </div>
    </div>
    <div class="panel-foot"><span>${esc(data.rodape)}</span><span>${esc(op.titulo)}</span></div>
  </div>`;
}

const sheet = (op) => `<div class="sheet">${panel(op)}${panel(op)}${panel(op)}</div>`;
...
const CSS = `
    ...
    * { box-sizing: border-box; }
    ...
    .panel-head.wm .logo { width:42mm; height:13mm; filter:none; }
    .panel-body { flex:1 1 auto; min-height:0; padding-top:4mm; position:relative; z-index:1; display:flex; flex-direction:column; }
    ...
    .sections { flex:1 1 auto; display:flex; flex-direction:column; gap:2.05mm; }
    .section { flex:1 1 auto; min-height:0; display:flex; flex-direction:column; justify-content:center; padding-top:.4mm; break-inside:avoid; }
...
console.log(`Menu de evento (${SLUG}) — ${data.opcoes.length} opções: ${data.opcoes.map((o) => o.titulo).join(', ')}`);
